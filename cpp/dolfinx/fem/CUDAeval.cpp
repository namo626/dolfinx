...
namespace dolfinx::fem
{
  ...
  void CUDAeval(std::span<const T> x, std::array<std::size_t, 2> xshape,
            std::span<const std::int32_t> cells, std::span<U> u,
                std::array<std::size_t, 2> ushape) {
    if (cells.empty())
      return;

    assert(x.size() == xshape[0] * xshe[1]);
    ...
    for (std::size_t p = 0; p < cells.size(); ++p)
    {
      ...
      else
      {
        basis_expansion<<<1, 1>>>(p, bs_element, space_dimension, value_size,
                                  ushape[1], u.data(), coefficients.data(),
                                  basis_values_b.data());
      }
    }
  }
